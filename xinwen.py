...
if st.button("快讯 - 富途牛牛"):
    ...
    for i in range(len(stock_info_global_futu_df)):
        st.markdown(f"[{stock_info_global_futu_df['标题'][i]}]({stock_info_global_futu_df['链接'][i]})")
...
if st.button("全球财经直播 - 同花顺财经"):
    ...
    for i in range(len(stock_info_global_ths_df)):
        st.markdown(f"[{stock_info_global_ths_df['标题'][i]}]({stock_info_global_ths_df['链接'][i]})")
...
if st.button("证券原创 - 新浪财经"):
    ...
    st.write(stock_info_broker_sina_df)
