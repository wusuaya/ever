...
# 获取A股日线数据的函数
def get_a_stock_data_daily(symbol, start_date, end_date, adjust):
    df = ak.stock_zh_a_hist(symbol=symbol, period="daily", start_date=start_date, end_date=end_date, adjust=adjust)
    df = df.rename(columns={"日期": "时间"})  # 重命名时间列
    df['时间'] = pd.to_datetime(df['时间'])  # 确保时间列为日期时间格式
    return df.reset_index(drop=True)

# 获取港股日线数据的函数
def get_hk_stock_data_daily(symbol, start_date, end_date, adjust):
    df = ak.stock_hk_hist(symbol=symbol, period="daily", start_date=start_date, end_date=end_date, adjust=adjust)
    df = df.rename(columns={"日期": "时间"})  # 重命名时间列
    df['时间'] = pd.to_datetime(df['时间'])  # 确保时间列为日期时间格式
    return df.reset_index(drop=True)
...
# Streamlit 应用
st.title("股票K线图与布林线（A股与港股支持 - 日线数据）")

# 输入参数
symbol = st.text_input('请输入股票代码', '300033')
start_date = st.text_input("选择开始日期 (格式: YYYYMMDD)", "20240101")
end_date = st.text_input("选择结束日期 (格式: YYYYMMDD)", "20241101")
adjust = st.selectbox("选择复权类型", ['', 'qfq', 'hfq'])

# 判断股票类型并获取数据
if is_hk_stock(symbol):
    st.write("识别为港股代码，正在获取港股日线数据...")
    df = get_hk_stock_data_daily(symbol, start_date, end_date, adjust)
else:
    st.write("识别为A股代码，正在获取A股日线数据...")
    df = get_a_stock_data_daily(symbol, start_date, end_date, adjust)
...
total_ratio = (total_hits / (2 * total_k_lines)) * 100 if total_k_lines > 0 else 0

for key, count in counts.items():
    ratio = (count / (2 * total_k_lines)) * 100 if total_k_lines > 0 else 0
    st.write(f"{key} 匹配次数: {count}, 占比: {ratio:.2f}%")
st.write(f"总匹配次数: {total_hits}, 总匹配比例: {total_ratio:.2f}%")
...
if st.button('寻找最佳窗口大小'):
    ...
    for w in range(3, 121):
        ...
        counts, _, _ = count_and_mark_bollinger_hits(temp_df, tolerance)
        if counts['中轨'] > max_matches:
            max_matches = counts['中轨']
            best_window = w
